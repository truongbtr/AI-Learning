...
/**
 * GET /api/tts?text=…&lang=vi-VN[&clip=key] → audio/mpeg, or 204 when no recorded clip exists and
 * cloud TTS is not configured (client falls back to Web Speech). Any signed-in user; text only —
 * no student data is sent to the provider (docs/00 privacy rule: nicknames only).
 */
export const GET = handle(async (request: Request) => {
  ...
  try {
    result = await getOrSynthesize(q.text, q.lang, q.clip);
  } catch (err) {
