...
- `.env.example` liệt kê: `DATABASE_URL`, `AUTH_SECRET`, `ADMIN_USERNAME`, `ADMIN_EMAIL`, `ADMIN_PASSWORD`, `TTS_PROVIDER` (mặc định `webspeech`), `TTS_API_KEY?`, `FILE_ROOT`, `INBOX_ROOT`, `TUNNEL_TOKEN`. **Không có khoá API LLM.**
